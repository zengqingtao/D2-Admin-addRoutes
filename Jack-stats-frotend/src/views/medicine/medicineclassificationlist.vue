...
export default {
  name: 'medicineclassificationlist',
  data () {
    return {
      firstList: null,
      secondList: null,
  ...
  methods: {
    ...
    fetchData () {
      ...
      getMedicineClassificationList({ 'page': 1, 'limit': 1000, 'type': 'disease', 'parent': 1 }).then(response => {
        this.firstList = response.data
        this.firstLoading = false
      })
      this.secondLoading = true
      getMedicineClassificationList({ 'page': 1, 'limit': 1000, 'type': 'disease', 'parent': 0 }).then(response => {
