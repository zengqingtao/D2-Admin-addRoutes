<template>
  <d2-container>
    ...
    <template slot="header">
      ...
      <el-input v-model="listQuery.search" @keyup.enter.native="handleFilter" style="width: 180px;margin-left: 10px;margin-right:20px;" placeholder="输入名称/身份证/订单号/诊疗卡号搜索" clearable></el-input>
      <el-date-picker
    ...
    <el-table :data="list"
      ...
      <el-table-column align="center" label="跟进人员" width="150">
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.general_name">转交全科：{{ scope.row.general_name }}</span><br/>
    ...
    <follow ref="follow" @handleHealth="handleHealth" @handleRefresh="fetchData"></follow>
...
<script>
import { getSplitList, getWorkByOrderId } from '@/api/orders'
...
import follow from '@/components/lianhe/follow'
...
export default {
  ...
  methods: {
    ...
    fetchData () {
      ...
      getSplitList(this.listQuery).then(response => {
