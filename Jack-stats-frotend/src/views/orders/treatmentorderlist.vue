  ...
  <d2-container>
    ...
    <template slot="header">
      <el-select v-model="listQuery.status" placeholder="请选择订单状态">
        ...
        <el-option
          v-for="item in treatmentOrderStatusOptions"
          :key="item.value"
          :label="item.label"
          :value="item.value">
        </el-option>
      </el-select>
      <el-input v-model="listQuery.search" @keyup.enter.native="handleFilter" style="width: 180px;margin-left: 10px;margin-right:20px;" placeholder="输入姓名/手机/诊疗卡号搜索" clearable></el-input>
      <el-date-picker
    ...
    <el-table :data="list"
      ...
      <el-table-column prop="status" label="状态" align="center" min-width="130">
        <template slot-scope="scope">
          <span>{{ scope.row.status | parseTreatmentOrderStatus }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogFormTitle" :visible.sync="dialogFormVisible" width="80%" center :close-on-click-modal="false">
      <el-card class="order-check-project-detail">
        ...
        <div class="mb2">
          <p class="h1">治疗项目：
           <!--  <el-button type="primary" @click="handleTreatmentList" v-if="order.status == 'pay'">添加</el-button> -->
          </p>
          <el-table :data="orderItems" show-summary :summary-method="getSummaries">
            ...
            <el-table-column prop="checkstore_name" align="center" label="检查门店" width="150"></el-table-column>
            <!-- <el-table-column label="操作" align="center" width="150">
              <template slot-scope="scope">
                <div v-if="order.status === 'pay' && order.status != 'cancelled'">
...
<script>
import { getTreatmentOrderList, getTreatmentOrderItems, setTreatmentOrder } from '@/api/orders'
import { getOrganizationsList, getCheckStoresList } from '@/api/hospital'
import { channelOptions, examinationsOptions, treatmentOrderStatusOptions } from '@/data/data'
...
import { hideTreatmentOrder } from '@/api/hidedel'
import { getICDList } from '@/api/neworder'
export default {
  name: 'treatmentorderlist',
  components: {
    treatment,
    timeperiod,
    paynow
  ...
  data () {
    return {
      ...
      icds: [],
      treatmentOrderStatusOptions: treatmentOrderStatusOptions,
      order: {},
      item: {},
      itemIndex: 0,
      orderItems: [],
      checkstoreList: [],
      selectedCheckstoreList: [],
      organizations: [],
      search_form: {
        'search': '',
        'organization_id': '',
      ...
      dialogTreatmentSetVisible: false,
      pickerOptions0: {
        disabledDate(time) {
  ...
  methods: {
    handleHide (row) {
      this.$confirm('确认隐藏此订单？', {
        ...
        hideTreatmentOrder({order_id: row.id}).then(response => {
          this.fetchData()
        })
      })
    },
    ...
    handlePay (order_id) {
      this.$refs.paynow.handlePayNow(order_id, 'App\\Models\\TreatmentOrder')
    },
    handleEdit (row) {
      this.dialogFormTitle = '治疗详情'
      this.order = row
      getTreatmentOrderItems({ 'order_id': row.id }).then(response => {
    ...
    filterOrganization (val) {
      this.selectedCheckstoreList = []
      for (var i in this.checkstoreList) {
        if (val === this.checkstoreList[i].organization_id) {
          this.selectedCheckstoreList.push({ 'id': this.checkstoreList[i].id, 'name': this.checkstoreList[i].name })  
        }
      }
      if (this.selectedCheckstoreList.length === 0) {
        this.search_form.checkstore = ''
      }
    },
    ...
    handleSaveOrderItems () {
      ...
      setTreatmentOrder(this.order).then(response => {
        Message.info('修改成功')
    ...
    handleTreatmentList () {
      ...
      for (var i in this.orderItems) {
        tmp.push(this.orderItems[i].treatment_id)
      }
    ...
    handleOrderAmount () {
      var tmp = 0
      for (var i in this.orderItems) {
        tmp += parseFloat(this.orderItems[i].price * this.orderItems[i].count) 
      }
    ...
    handleItem (item) {
      this.item = item
      this.dialogTreatmentSetVisible = true
    },
    handleFilter () {
      this.listQuery.page = 1
      this.fetchData()
    ...
    fetchData () {
      ...
      getTreatmentOrderList(this.listQuery).then(response => {
    ...
    fetchCheckstore () {
      getOrganizationsList({ 'page': 1, 'limit': 20 }).then(response => {
        this.organizations = response.data
      })
      getCheckStoresList({ 'page': 1, 'limit': 500 }).then(response => {
        this.checkstoreList = response.data.list
      })
    }
  }
}
