    ...
    <el-table :data="list"
      ...
      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="handleEdit(scope.row)">查看/编辑</el-button>
    ...
    <el-dialog :title="dialogFormTitle" :visible.sync="dialogFormVisible" width="80%" center :close-on-click-modal="false">
      <el-form ref="form" :model="form">
        ...
        <el-form-item label="治疗项目">
          <el-button @click="handleTreatmentList">添加</el-button>
        </el-form-item>
        <el-table :data="form.items">
            ...
            <el-table-column prop="organization_name" align="center" label="治疗机构" width="130"></el-table-column>
            <el-table-column prop="price" align="center" label="零售价" width="150"></el-table-column>
            <el-table-column prop="count" align="center" label="次数" width="150">
              <template slot-scope="scope">
                <el-input type="text" v-model="scope.row.count" center></el-input>
              </template>
            </el-table-column>
...
<script>
import { getCommonTreatmentList, setCommonTreatment, getCommonTreatment } from '@/api/hospital'
import { Message } from 'element-ui'
import { regionData, CodeToText } from 'element-china-area-data'
import treatment from '@/components/lianhe/treatment'
import { hideCommonTreatment } from '@/api/hidedel'
import { getICDList } from '@/api/neworder'
export default {
  name: 'commontreatmentlist',
  components: {
    treatment
  },
  data () {
  ...
  methods: {
    handleHide (row) {
      this.$confirm('确认隐藏此通用治疗单？', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        hideCommonTreatment({order_id: row.id}).then(response => {
          this.fetchData()
        })
      })
    },
    ...
    handleCreate () {
      ...
      this.dialogFormTitle = '新增通用治疗单'
    },
    handleEdit (row) {
      this.dialogFormTitle = '修改通用治疗单'
      this.dialogFormVisible = true
      getCommonTreatment({id: row.id}).then(response => {
    ...
    fetchData () {
      ...
      getCommonTreatmentList(this.listQuery).then(response => {
