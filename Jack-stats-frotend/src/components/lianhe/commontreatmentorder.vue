<template>
  <el-dialog :visible.sync="dialogVisible" title="通用通用治疗单" width="35%" :loading="loading">
    <el-table :data="list" element-loading-text="拼命加载中" stripe v-loading="loading">
      ...
      <el-table-column align="center" label="操作" width="250">
        <template slot-scope="{$index, row}">
          ...
          <el-button type="text" @click="addToOrder(row.id)">加入治疗单</el-button>
...
<script>
import { getCommonTreatmentList } from '@/api/hospital'
  ...
  methods: {
    ...
    handleHide (id) {
      this.$confirm('确认隐藏此通用治疗单？', {
        ...
        hideCommonTreatment({'order_id': id}).then(response => {
    ...
    showCommon(type) {
      ...
      getCommonTreatmentList(this.search_form).then(response => {
