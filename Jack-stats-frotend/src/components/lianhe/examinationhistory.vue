<template>
  <el-dialog :visible.sync="dialogVisible" title="历史检查单" width="90%" :loading="loading">
...
<script>
import { getExaminationHistoryList } from '@/api/neworder'
export default {
  data() {
    return {
      dialogVisible: false,
  ...
  methods: {
    ...
    showHistory(patient_id) {
      ...
      getExaminationHistoryList({patient_id: patient_id, page: this.search_form.page, limit: 20}).then(response => {
