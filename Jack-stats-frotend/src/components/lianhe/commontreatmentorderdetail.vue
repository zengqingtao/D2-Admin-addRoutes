<template>
  <el-dialog :visible.sync="dialogVisible" title="通用通用治疗单详情" width="90%" :loading="loading">
...
<script>
import { getCommonTreatment } from '@/api/hospital'
export default {
  data() {
    return {
      id: null,
  ...
  methods: {
    commonDetail(id) {
      this.loading = true;
      // API.get('common-treatment-orders/' + this.id).then((res) => {
      //   this.form = res;
      // });
