<template>
  <div>
    <el-dialog title="治疗项目" :visible.sync="dialogFormVisible" width="80%" center :close-on-click-modal="false">
      <!-- header 查询条件 -->
      <div>
        ...
        <el-select v-model="listQuery.checkstores" placeholder="请选择门店">
          <el-option label="全部" value=""></el-option>
          <el-option
...
<script>
import { getTreatmentsList, getOrganizationsList, getCheckStoresList } from '@/api/hospital'
...
export default {
  ...
  methods: {
    ...
    handleAdd (row) {
      ...
      this.$emit('handleAddTreatment', {'id': '', 'code': row.code, 'name': row.name, 'sample': row.sample, 'type': row.type, 'price': row.price, 'detail': row.detail, 'source': row.source, 'treatment_id': row.id, 'treatment_order_id': '', 'organization_id': row.organization_id, 'shop_code': this.listQuery.checkstores, 'date': '', 'time_period': '', 'oranization_name': row.organization_name, 'checkstore_name': this.checkstore_name, 'count': 1})
    },
