...
// import { GetMenu } from '@/myApi/menu'
...
router.beforeEach((to, from, next) => {
  if (validatenull(store.state.d2admin.user.accessToken) && !validatenull(util.getToken())) {
    ...
    GetMenu().then(res => {
      ...
      console.log("util.formatRoutes(res):",util.formatRoutes(res));
      ...
      // 设置侧边栏菜单
      store.commit('d2admin/menu/asideSet', res.data.data)
      // 设置顶栏菜单
      store.commit('d2admin/menu/headerSet', res.data.data)
      router.addRoutes(oRoutes)
      if (util.checkRoute(res.data.data, to.path)) {
        next({ path: to.path })  
      } else {
        next({ name: 'index' })
      }
